resource "aws_s3_bucket_object" "upload_folder" {
  for_each = fileset(var.local_folder, "**/*")

  bucket       = var.s3_bucket_id
  key          = var.s3_folder != null ? "${var.s3_folder}/${each.value}" : each.value
  source       = "${var.local_folder}/${each.value}"
  etag         = filemd5("${var.local_folder}/${each.value}")
  content_type = lookup(local.content_type, regex("\\.[^.]+$", each.value), null)
}
